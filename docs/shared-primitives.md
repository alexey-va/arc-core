...
| Paper inventory sessions | `arc-core-paper-menu`: `PaperMenuService`, `PaperMenuContent`, `PaperMenuItemFactory`, `PaperMenuTextContract` | Inventory Framework is internal. YAML owns safe item presentation including MiniMessage name/lore composition, conditions and repeated rows. Code supplies typed component tags and actions. Cancel inventory mutation, except an explicit domain-reserved `PaperMenuTransferHandler` top-slot pickup; dispatch typed semantic targets once, reject invalid text/region overflow before mutation, invalidate stale generations and delayed feedback, and close the service at shutdown. |
