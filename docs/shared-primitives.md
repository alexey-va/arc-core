...
| Composed player nameplate rows | `arc-core`: `PlayerNameplateRegistry`, `NameplateLayer`; `arc-core-paper`: `PaperPlayerNameplates`; `arc-core-paper-testing`: `RecordingPaperNameplateDisplayFactory` | Gameplay owns one styled `Component` per stable row. Core bounds and composes rows; Paper owns one transient TextDisplay per target, per-viewer distance/visibility/line-of-sight checks, passenger recovery, and terminal cleanup. |
